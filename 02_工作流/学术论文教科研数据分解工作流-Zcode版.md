...
status: active
version: 1.2.0
source: 融合同名论文完整拆解工作流的七类数据域命名/统一证据状态/一票否决项，保留zcode四类型分流与阶段门体系
tags:
...
本工作流是 [[学术论文教科研数据分解工作流-Zcode版]] 的"学术论文专项一站式落地"。与已有两个工作流的关系:
...
- **上位**: [[学术论文教科研数据分解工作流-Zcode版]] 提供通用 10 阶段框架、十层级原始参照、复核对照材料、入库分发规则。
...
| 状态 | 含义 | 示例 |
|---|---|---|
| 实际呈现 | 论文中存在具体数据、表图、原话、材料或工具 | 表3报告了样本均值和标准差 |
| 作者声称 | 作者说使用或获得，但没有给出可复核内容 | "收集了学生成绩"但结果未呈现 |
| 设计预期 | 方案预计产生，不能证明已经发生 | 教案预计学生完成原型测试 |
| 研究应有 | 按研究问题或方法应当存在，但论文未报告 | 多评分者研究应报告一致性 |
| 分析者计算 | 根据论文明确数值重新计算 | 根据各档人数计算总合格率 |
| 分析者解释 | 对方法、证据强度、风险或复用的判断 | 无对照，因此因果支持弱 |
| 缺失/不可读 | 当前材料不存在或无法可靠识别 | 扫描表格数值模糊 |

分析者计算必须给出公式、输入值和证据位置；不得与作者原始结果混在同一字段。本表与第二节"数据三态"的关系：三态是本表的状态1/2/3的简称，本表补全为七态。
...
按 [[学术论文教科研数据分解工作流-Zcode版]] 阶段 0 执行,补充:
...
按 [[学术论文教科研数据分解工作流-Zcode版]] 阶段 2 执行,补充:
...
产物分发表(沿用 [[学术论文教科研数据分解工作流-Zcode版]] 阶段 8,补充学术论文特有落位):
...
出现任一项，不得标记 `completed`：

- 编造或补全论文中不存在的数据、样本、统计量、参考文献或页码。
- 把作者声称使用的数据写成实际呈现的数据。
- 把设计预期、教学范例或示意图写成真实实施结果。
- 核心结论找不到论文中的证据位置。
- 把单班、相关或自陈结果写成强因果证据。
- 删除或掩盖与作者结论不一致的数据。
- 未建立独立复核记录。
...
| 工作流 | 关系 | 调用点 |
|---|---|---|
| [[学术论文教科研数据分解工作流-Zcode版]] | 上位通用框架 | 阶段 0、1、8、9 引用其通用规则、十层级参照、复核对照材料、入库分发 |
| [[学位论文参考文献与正文引用映射工作流]] | 参考文献专项 | 阶段 3 直接调用其阶段 2-7,可选延伸调用其阶段 8-9 |
| 本工作流 | 学术论文一站式完整分解 | 整合 7 大数据域,规定四类型分流和总报告结构 |

> [!important] 定位声明
> 本工作流不替代上述两个工作流,而是在它们之上提供"一篇论文丢进来,所有数据都分析出来"的一站式执行路径。遇到参考文献专项问题以 [[学位论文参考文献与正文引用映射工作流]] 为准,遇到通用框架问题以 [[学术论文教科研数据分解工作流-Zcode版]] 为准。
