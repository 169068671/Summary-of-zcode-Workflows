...
const DEFAULT_SETTINGS = {
  remoteUrl: "https://github.com/169068671/Summary-of-zcode-Workflows.git",
  branch: "main",
  runValidation: true,
};

class ZcodeGitHubSyncPlugin extends Plugin {
  async onload() {
    this.settings = Object.assign({}, DEFAULT_SETTINGS, await this.loadData());
    this.syncing = false;
    this.statusBar = this.addStatusBarItem();
    this.statusBar.setText("GitHub: 待同步");
    this.addRibbonIcon("cloud-upload", "同步 Zcode 工作流仓库到 GitHub", () => this.runSync(false));
    this.addCommand({
      id: "sync-zcode-workflows-to-github",
      name: "一键同步整个仓库到 GitHub",
      callback: () => this.runSync(false),
    });
    this.addCommand({
      id: "check-zcode-workflows-github-status",
      name: "检查 GitHub 同步状态",
  ...
  async runSync(statusOnly) {
    ...
    try {
      ...
      console.error("Zcode 工作流 GitHub Sync failed", error);
...
class SyncSettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Zcode 工作流 GitHub Sync" });
    containerEl.createEl("p", { text: "插件不保存 Token，使用电脑现有的 Git 凭据；不会强推或改写历史。" });
    new Setting(containerEl)
      .setName("GitHub 仓库地址")
      .setDesc("远程地址不一致时停止，不会自动替换。")
    ...
    new Setting(containerEl)
      .setName("上传前核验知识库")
      .setDesc("建议始终开启。")
      .addToggle((toggle) => toggle.setValue(this.plugin.settings.runValidation).onChange(async (value) => {
...
module.exports = ZcodeGitHubSyncPlugin;
