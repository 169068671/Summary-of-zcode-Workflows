...
CORE_DIRS = {"01_长期记忆", "02_工作流", "03_决策记录", "04_项目"}
...
def tags(block: str) -> list[str]:
    ...
    if match:
        return [item.strip() for item in re.findall(r"^\s+-\s+(.+)$", match.group(1), re.M)]
...
def main() -> int:
    ...
    if not tag_index_text:
        errors.append("缺少标签索引")
    if not content_index_text:
        errors.append("缺少内容索引")
    ...
    for path in markdown_files:
        ...
        for raw_link in re.findall(r"!?\[\[([^\]]+)\]\]", text):
            if resolve_wikilink(path, raw_link, markdown_files) is None:
                errors.append(f"{rel}: 断链 [[{raw_link}]]")

        if rel.parts and rel.parts[0] in CORE_DIRS and path.stem not in content_index_text:
            errors.append(f"{rel}: 未登记到内容索引")

        if re.search(r"\b(?:sk|key)-[A-Za-z0-9_.-]{20,}\b", text):
            errors.append(f"{rel}: 疑似包含明文 API 密钥")

    for tag in sorted(used_tags):
        if f"#{tag}" not in tag_index_text and f"`{tag}`" not in tag_index_text:
            errors.append(f"标签 #{tag} 未登记到标签索引")

    for config in sorted((ROOT / ".obsidian").glob("*.json")):
        try:
            json.loads(config.read_text(encoding="utf-8"))
        except Exception as exc:
            errors.append(f"{config.relative_to(ROOT)}: JSON 无效 ({exc})")
